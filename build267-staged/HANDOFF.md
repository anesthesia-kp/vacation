...
### 2 · AFTER the rehearsal completes, FIRST: build 268 — Firestore-rule hardening (owner-ordered sequencing: ship this BEFORE the final audit, so the audit examines the system in its true final form)
Server-side freeze of `auctionConfig` + `bidFloors` once Phase 1 begins (today the lock is client-side only; this converts a promise into a guarantee). The ONE hardening recommended for shipping. Full gates + honesty tests as always. NOTE: `firestore.rules` deploys through the Firebase console (owner action), not the GitHub Pages push — plan the rules-publish step explicitly and verify with the rules test suite before AND after.

### 3 · THEN: THE FINAL EXHAUSTIVE MULTI-CLAUDE ADVERSARIAL AUDIT (owner-requested, the next chat's main event)
...
### 4 · LAUNCH-EVE (after the audit) — the closing ritual
1. **Build ~269 ONLY IF the audit confirmed findings** (triage critical/high vs deferred-minor; owner decides).
